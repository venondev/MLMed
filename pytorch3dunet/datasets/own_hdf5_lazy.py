...
logger = get_logger('OwnH5LazyHDF5Dataset')
...
class OwnLazyHDF5Dataset(ConfigDataset):
    ...
    def __init__(self, file_path,
        ...
        self.artery_internal_path = artery_internal_path
        ...
        if global_normalization:
            ...
            self.stats = {'pmin': None, 'pmax': None, 'mean': None, 'std': None}
        ...
        if phase != 'test':
            ...
            label = self.fetch_and_check(self.input_file, label_internal_path)

            if weight_internal_path is not None:
                # look for the weight map in the raw file
                weight_map = self.fetch_and_check(self.input_file, weight_internal_path)
                self.weight_transform = self.transformer.weight_transform()
                self.has_weight_map=True
            else:
                weight_map = None
        ...
        #         else:
        #             raw = np.pad(self.raw, pad_width=pad_width, mode='reflect')
